...
LoginWindow::LoginWindow(float width, float height)
    ...
    // Configurare text introdus pentru username
    ...
    // Configurare text introdus pentru password
...
/**
 * Desenează elementele ferestrei de autentificare.
 */
...
void LoginWindow::handleInput(sf::RenderWindow& window) {
    ...
    while (window.pollEvent(event)) {
        ...
        // Gestionare click pentru activarea casetelor
...
/**
 * Gestionează introducerea textului pentru caseta activă.
 */
void LoginWindow::handleTextInput(const sf::Event& event, sf::Text& target) {
    if (event.text.unicode < 128 && event.text.unicode != 13 && event.text.unicode != 8) { // Exclude Enter și Backspace
        target.setString(target.getString() + static_cast<char>(event.text.unicode));
    }
    else if (event.text.unicode == 8 && target.getString().getSize() > 0) { // Backspace
...
/**
 * Gestionează introducerea textului pentru caseta de parolă.
 */
void LoginWindow::handlePasswordInput(const sf::Event& event) {
    if (event.text.unicode < 128 && event.text.unicode != 13 && event.text.unicode != 8) { // Exclude Enter și Backspace
        m_passwordValue += static_cast<char>(event.text.unicode); // Salvează caracterul în parola reală
        m_password.setString(std::string(m_passwordValue.size(), '*')); // Afișează asteriscuri
    }
    else if (event.text.unicode == 8 && !m_passwordValue.empty()) { // Backspace
        m_passwordValue.pop_back(); // Șterge ultimul caracter din parola reală
        m_password.setString(std::string(m_passwordValue.size(), '*')); // Actualizează asteriscurile
    }
}
